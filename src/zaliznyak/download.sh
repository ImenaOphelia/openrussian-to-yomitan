...
mkdir -p "../../assets/dictionary/Нарицательные"
mkdir -p "../../assets/dictionary/Глаголы"
...
for L in $ALPHABET; do
    ...
    curl -s -f -L -o "../../assets/dictionary/Нарицательные/${L}.txt" "$URL"
    if [ $? -eq 0 ]; then
        echo "  Saved ${L}.txt"
    fi
done
...
for L in $VERB_LETTERS; do
    ...
    curl -s -f -L -o "../../assets/dictionary/Глаголы/${L}.txt" "$URL"
    if [ $? -eq 0 ]; then
        echo "  Saved ${L}.txt"
    fi
done
...
curl -s -f -L -o "../../assets/dictionary/Собственные.txt" "$PROPER_URL"
if [ $? -eq 0 ]; then
